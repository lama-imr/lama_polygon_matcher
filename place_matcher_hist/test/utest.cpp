...
geometry_msgs::Polygon getTestPolygon()
{
  ...
  std::vector<geometry_msgs::Point32> points;
  points = interpolate(p0, p1, 0.02);
  std::copy(points.begin(), points.end(), std::back_inserter(poly.points));
  points = interpolate(p1, p2, 0.02);
  std::copy(points.begin(), points.end(), std::back_inserter(poly.points));
  points = interpolate(p2, p3, 0.02);
  std::copy(points.begin(), points.end(), std::back_inserter(poly.points));
  points = interpolate(p3, p4, 0.02);
  std::copy(points.begin(), points.end(), std::back_inserter(poly.points));
  points = interpolate(p4, p0, 0.02);
  std::copy(points.begin(), points.end(), std::back_inserter(poly.points));
...
/**
 * Data from CTU, IMR internal dataset "real-2sicks-karlovo-nam-e"
 * file wca-2.txt. The last 2 points are excluded because they cause a
 * non-simple polygon.
 */
geometry_msgs::Polygon getRealPolygon()
{
  std::stringstream poly_sstream;
  poly_sstream <<
    "0 1.116" <<
    "0.019634 1.12483\n" <<
    "0.039925 1.1433\n" <<
  ...
  geometry_msgs::Polygon poly = loadFromString(poly_sstream);
  return poly;
}

TEST(TestSuite, testGetDissimilarity)
{
  const geometry_msgs::Polygon poly_ref = getRealPolygon();
  const double dtheta = 1.3;
  ...
  ros::ServiceClient client = g_nh_ptr->serviceClient<place_matcher_msgs::PolygonDissimilarity>("/place_matcher_hist/compute_dissimilarity");
  ...
  // Don't expect it to be better than the translational and rotational resolutions.
  EXPECT_NEAR(res.pose.position.x, transform.position.x, 1.1 * 0.1);
  EXPECT_NEAR(res.pose.position.y, transform.position.y, 1.1 * 0.1);
  EXPECT_NEAR(tf::getYaw(res.pose.orientation), dtheta, 1.1 * 0.01745);
}
...
int main(int argc, char *argv[])
{
  ...
  ros::init(argc, argv, "test_cpp_place_matcher_hist");
  ros::NodeHandle nh;
  g_nh_ptr.reset(&nh);
